...
class MarketSavePriceStock extends Command
{
    protected static $defaultName = 'cron_prepared:market_product_priceStock_save';
    ...
    {
        $this
            ->setDescription('Run market product priceStock save')
            ->setHelp('This command allows you to market product priceStock save...')
            ->addOption(
                'retry',
                'retry',
        ...
        try {
            if ($this->condition->checkBusy()) {
                ...
                $response = $this->connector->getContent($token, 'PlantsMarket', $input->getOption('retry'),
                    'v1/channel/' . $channel . '/product/stock-price');
